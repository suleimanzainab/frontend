...
const actions = {
  login: (context, payload) => {
    ...
    axios({
      ...
      .then((res) => {
        ...
        context.commit("mutateToken", res.data.access);
        context.dispatch("get_user_profile", res.data.access);
      })
      .catch((err) => {
  ...
  get_user_profile: (context) => {
    ...
    axios
      .get(`${process.env.VUE_APP_BACKEND_URL}/user/profile/`, {
        headers: {
  ...
  register: async (context, payload) => {
    try {
      const response = await axios.post(
        `${process.env.VUE_APP_BACKEND_URL}/user/signup/`,
        payload
      );
      alert.success(
